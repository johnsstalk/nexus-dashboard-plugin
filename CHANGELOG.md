...
- **Stack blocks** — new `stack:` code block keyword for vertical stacking layout (replaces old `tabs:` system)
...
- **Replaced Tabs with Stack** — `tabs:` block type removed; replaced by `stack:` for vertical stacking
...
- `DashboardBlock` union type simplified — `RowConfig.children` narrowed to `(SectionConfig | StackConfig)[]`
